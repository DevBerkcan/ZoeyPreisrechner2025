...
export default function ControlPlatformPage() {
  ...
  const handleNameChange = (name: string) => {
    setNewTenant({ ...newTenant, name, slug: generateSlug(name) });
  };
  ...
  const createTenant = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error(error.error || "Fehler beim Erstellen");
      }
    } catch (error) {
  ...
  const deleteTenant = async () => {
    ...
    try {
      await fetch(`/api/tenants?id=${confirmDialog.tenantId}`, { method: "DELETE" });
      setTenants(tenants.filter((t) => t.id !== confirmDialog.tenantId));
    } catch (error) {
      console.error("Fehler:", error);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center gap-1 text-emerald-600 hover:underline">
              <ArrowLeft size={18} />
              Zurück
            </Link>
            <div className="flex items-center gap-2">
              <Building2 size={24} className="text-emerald-600" />
              <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Control Platform</h1>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{totalTenants}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{totalServices}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <p className="text-sm text-gray-500 dark:text-gray-400">Kunden</p>
            <p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{totalCustomers}</p>
          </div>
        </div>
        ...
        {showAddForm && (
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 mb-6 border border-gray-200 dark:border-gray-700">
            <h2 className="font-semibold text-lg mb-4 text-gray-800 dark:text-gray-100">Neuen Mandanten anlegen</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Slug (URL-Pfad) *</label>
                <div className="flex items-center gap-2">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Hauptfarbe</label>
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    value={newTenant.mainColor}
                    onChange={(e) => setNewTenant({ ...newTenant, mainColor: e.target.value })}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Sekundärfarbe</label>
                <div className="flex items-center gap-2">
                  <input
                    type="color"
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Logo URL (optional)</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">E-Mail (optional)</label>
                <input
                  type="email"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Telefon (optional)</label>
                <input
                  type="tel"
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
              <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-3">Erster Zugang (optional)</h3>
            ...
            <div className="flex justify-end gap-3 mt-4">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
              >
                <X size={16} />
                Abbrechen
              </button>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700 text-left text-sm text-gray-600 dark:text-gray-300">
                  ...
                  <th className="px-4 py-3 text-center">Zugänge</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {filteredTenants.map((tenant) => (
                  <tr
                    key={tenant.id}
                    className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 ${!tenant.isActive ? "opacity-60" : ""}`}
                  >
                    <td className="px-4 py-3">
                      <button
                        ...
                        title={tenant.isActive ? "Klicken zum Deaktivieren" : "Klicken zum Aktivieren"}
                      >
                        {toggling === tenant.id ? (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        ...
                        <div
                          ...
                          title={`Sekundär: ${tenant.secondaryColor}`}
                        />
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="font-medium text-gray-800 dark:text-gray-100">{tenant.name}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          title={tenant.isActive ? "Deaktivieren" : "Aktivieren"}
                        >
                          <Power size={16} />
                        </button>
                        <button
                          onClick={() =>
                            setConfirmDialog({ open: true, tenantId: tenant.id, tenantName: tenant.name })
                          }
      ...
      <ConfirmDialog
        ...
        onOpenChange={(open) =>
          setConfirmDialog({ open, tenantId: open ? confirmDialog.tenantId : null, tenantName: open ? confirmDialog.tenantName : "" })
        }
        title="Mandant löschen"
        description={`"${confirmDialog.tenantName}" wirklich löschen? Alle Services, Kunden und Zugänge werden unwiderruflich gelöscht!`}
